...
app.get("/admin/reports", function(req, res) {
  ...
  db.ref("reports").orderByChild("ts").once("value").then(function(snap) {
    ...
    snap.forEach(function(child) {
      reports.unshift(Object.assign({ id: child.key }, child.val()));
    });
    res.json(reports);
  });
});

// ── SUBMIT REPORT (called from frontend) ──
...
app.post("/admin/ignore/:id", function(req, res) {
  if (req.headers["x-admin-key"] !== ADMIN_PASSWORD) return res.status(401).json({ error: "unauthorized" });
  db.ref("reports/" + req.params.id).update({ status: "ignored" }).then(function() {
    res.json({ ok: true });
  });
});
...
app.post("/admin/unban/:uid", function(req, res) {
  if (req.headers["x-admin-key"] !== ADMIN_PASSWORD) return res.status(401).json({ error: "unauthorized" });
  db.ref("bans/" + req.params.uid).remove().then(function() {
    res.json({ ok: true });
  });
});
...
app.get("/admin/bans", function(req, res) {
  if (req.headers["x-admin-key"] !== ADMIN_PASSWORD) return res.status(401).json({ error: "unauthorized" });
  db.ref("bans").once("value").then(function(snap) {
...
app.listen(PORT, function() { console.log("void server running on port " + PORT); });
